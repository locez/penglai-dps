...
class 循环主类 {
  ...
  添加buff({
    ...
    const 新buff对象 = {
      ...
      当前层数: 覆盖层数
        ...
        : Math.min(
            (当前层数 || 0) + 新增层数,
            this.Buff和Dot数据[名称].最大层数 || 1
          ),
      刷新时间: 事件时间,
    };
    if (
      (新buff对象.当前层数 !== 当前层数 && 新buff对象.当前层数 !== 1) ||
      名称 === "标鹄"
    ) {
      this.添加战斗日志({
  ...
  技能造成伤害(
    ...
    const 技能增益列表 =
      技能原始数据?.find((item) => item.技能名称 === 来源)?.技能增益列表 || [];
    const 有关的buff列表 =
      技能增益列表
        ?.filter((item) => {
  ...
  技能释放前(当前技能: 循环基础技能数据类型, 技能实例, i) {
    ...
    const 延迟等待 =
      this.当前时间 && 当前技能?.技能GCD组 !== "自身" ? this.网络延迟 : 0;
  ...
  清空buff调用函数(对象: "自身" | "目标") {
    const buff列表 =
      对象 === "自身" ? this.当前自身buff列表 : this.当前目标buff列表;

    // 更新目标buff
    Object.keys(buff列表).forEach((key) => {
      const buff对象 = buff列表[key];
      const buff应该消失时间 =
        (buff对象?.刷新时间 || 0) + (buff对象?.最大持续时间 || 0);
  ...
  删除待生效事件队列(事件名称) {
    const 新待生效事件队列: 待生效事件[] = [
      ...(this.待生效事件队列 || []),
    ].filter((item) => item.事件名称 !== 事件名称);
    // 由于不确定待生效事件时间分布，重新排序
    新待生效事件队列.sort((a, b) => {
      return (a?.事件时间 || 0) - (b?.事件时间 || 0);
    });
  ...
  召唤宠物(额外buff?) {
    const 本次事件召唤索引 =
      this.召唤宠物索引 + 1 > 5 ? 0 : this.召唤宠物索引 + 1;
  ...
  获得承契() {
    if (this.校验奇穴是否存在("襄尺")) {
      ...
      if (buff承契数据) {
        const 应该消失时间 =
          (buff承契数据.刷新时间 || 0) + (buff承契数据.最大持续时间 || 0);
  ...
  模拟() {
    for (let i = 0; i < this.测试循环.length; i++) {
      this.本轮模拟开始前();
      const 当前技能 = this?.技能基础数据?.find(
        (item) => item?.技能名称 === this.测试循环[i]
      );
      if (当前技能) {
